...
function SignUp() {
  // State to manage form data and alert messages
  const [formData, setFormData] = useState({
    name: '',
    emailOrPhone: '',
    password: '',
  ...
  const handleChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    setFormData({
      ...formData,
      [e.target.name]: e.target.value, // Update the respective field's value
    });
  };
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    setAlertMessage('Account created successfully! 🎉'); // Show success message
    setFormData({
      name: '',
      emailOrPhone: '',
      password: '',
  ...
  return (
    <div className="flex flex-col sm:flex-row min-h-screen">
      {/* Left Section (Image) */}
      <div className="w-full sm:w-1/2 flex justify-center items-center p-4">
        {/* Displaying an image (e.g., related to cart or shop) */}
        <Image
          src="/images/cart.png"
          alt="Cart"
          width={500}
          height={500}
          className="object-contain"
          priority // Priority loading for the image
        />
      </div>
      ...
      <div className="w-full sm:w-1/2 flex justify-center items-center bg-white p-6 sm:p-12">
        <div className="w-full max-w-md">
          ...
          {/* Display Alert Message after successful account creation */}
          {alertMessage && (
            <div className="mt-4 text-center text-black">
              <p>{alertMessage}</p>
            </div>
